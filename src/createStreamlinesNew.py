...
# global initialization two things 1, set it to be the segments 2 subtract it from the low res does that change after that besides after each time step
# this is only used in extract streamlines and calc error.
# I set ut in main for each time step as segments
gt_segments = None  

# for 2d Ground truth only 
# I am sending the x,y and offsets of the segments of the grond truth here
...
def calc_error(streamline_point, cnt, alpha=0.005):
    ...
    if gt_segments is None:
        print(f"No ground truth available for step {cnt}. Returning point as is.")
        return streamline_point  

    # ground truth at this step and point 
    gt_x, gt_y = gt_segments[0][cnt] 

    error = np.sqrt((gt_x - streamline_point[0])**2 +
                    (gt_y - streamline_point[1])**2)
    if error > alpha:
        print(f"Error exceeds threshold: {error:.6f} > {alpha}. Adjusting point.")
        return (gt_x, gt_y)
    else:
        return tuple(streamline_point)
...
def rk4_streamline_from_grid(
    ...
    for i in range(len(x0)):
        ...
        for _ in range(max_steps):
            ...
            x, y = calc_error(np.array([x, y]), cnt)
            coords_x.append(x)
            coords_y.append(y)
...
def parse_arguments():
    ...
    # 2d only for right now -------------------------------------------------
    parser.add_argument("--bp_file_high", "-hr", required=True, help="High resoltuion data")
    parser.add_argument("--var_x", "-vx", required=True, help="x component of the high res data")
    parser.add_argument("--var_y", "-vy", required=True, help="y component of the high res data")
    parser.add_argument("--var_offset", "-vo", required=True, help="offsets variable of the high res data")
...
def main():
    ...
    bp_file_high = args.bp_file_high
    ...
    r_gt = ReaderClass.Reader(
        IO_Name=io_name, bp_file=bp_file_high, xml=xml_file
    )
    ...
    while True:
        ...
        r_gt.begin_step()
        r_gt.set_read_vars([args.var_x, args.var_y, args.var_offset])
        segmentX = r_gt.read_step(args.var_x)
        segmentY = r_gt.read_step(args.var_y)
        segmentd_offset = r_gt.read_step(args.var_offset)
        
        r_gt.end_step()

        # this is one time step of the exact streamline. It updates after each time step
        # NOTE: Below is very very very imprtant
        # I am assuming that the low res strealine that I am making will have the same number of RK4 steps. 
        # -----------------------------------------------------------------------------------------
        global gt_segments
        gt_segments = extract_streamlines_from_segments(segmentX, segmentY, segmentd_offset)
        print("Extracted GT streamlines")
        #  now you can extract the streamlines from the segments ------------------------------
