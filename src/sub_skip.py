...
def parse_arguments():
    ...
    parser.add_argument("--var1", required=True, help="Variable name from the first file")
    parser.add_argument("bpfile2", help="Second input BP file (high res, ground truth)")
    parser.add_argument("--var2", required=True, help="Variable name from the second file")
    parser.add_argument(
        "--output_file", 
    ...
    parser.add_argument(
        "--xml", 
        default=None, 
        help="Optional ADIOS2 XML configuration file"
    )
    parser.add_argument(
        "--max_steps", 
        type=int,
...
def main():
    ...
    with Stream(io1, low_res_file, "r") as f1, \
         Stream(io2, high_res_file, "r") as f2, \
         Stream(io_out, args.output_file, "w") as fout:
        ...
        while True:
            ...
            for i in range(ny_low):
                for j in range(nx_low):
                    gt_i = int(i * skip_y)
                    gt_j = int(j * skip_x)
                    truncation_error[i, j] = data_high_2d[gt_i, gt_j] - data_low_2d[i, j]
            ...
            fout.write(f"{args.var1}_truncation_error", truncation_error, output_shape, [0, 0], output_shape)
            fout.end_step()
