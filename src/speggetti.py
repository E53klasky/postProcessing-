...
def extract_streamlines(x_coords, y_coords, offsets):
    ...
    for i in range(len(offsets) - 1):
        ...
        if end > start:
            streamlines.append(
                np.column_stack((x_coords[start:end], y_coords[start:end]))
            )
    return streamlines
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Plot ensemble (spaghetti) streamlines"
    )
...
def main():
    ...
    while True:
        ...
        if (
            reader.vars_Out.get(args.var_x) is None
            or reader.vars_Out.get(args.var_y) is None
            or reader.vars_Out.get(args.var_offset) is None
        ):
        ...
        ax.set_title(
            f"Ensemble Streamlines — Step {current_step}  ({len(streamlines)} lines)"
        )
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.set_aspect("equal")
