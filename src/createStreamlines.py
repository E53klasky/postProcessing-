...
def rk4_streamline_from_grid(
    ...
    if is_3d:
        ...
        if use_trunc:
            ...
            interp_ex = RegularGridInterpolator((ygrid_e, xgrid_e), vx_trunc, method="cubic")
            interp_ey = RegularGridInterpolator((ygrid_e, xgrid_e), vy_trunc, method="cubic")
            interp_ez = (
                RegularGridInterpolator((ygrid_e, xgrid_e), vz_trunc, method="cubic")
                if vz_trunc is not None else None
            )

        def vector_field(x, y, z, perturb=False):
        ...
        def vector_field(x, y, perturb=False):
            point = np.array([y, x])
            try:
                u = float(interp_vx(point).item())
                v = float(interp_vy(point).item())
    ...
    for i in range(len(x0)):
        for _ensemble in range(num_ensemble):
            ...
            for _ in range(max_steps):
                ...
                if is_3d:
                    k1 = vector_field(x,            y,            z,            perturb=True)
                    k2 = vector_field(x+dt*k1[0]/2, y+dt*k1[1]/2, z+dt*k1[2]/2, perturb=True)
                    k3 = vector_field(x+dt*k2[0]/2, y+dt*k2[1]/2, z+dt*k2[2]/2, perturb=True)
                    k4 = vector_field(x+dt*k3[0],   y+dt*k3[1],   z+dt*k3[2],   perturb=True)
                    dx, dy, dz = dt / 6 * (k1 + 2*k2 + 2*k3 + k4)
                    x_prev, y_prev, z_prev = x, y, z
                    x += dx; y += dy; z += dz
                    arc_len += np.sqrt((x-x_prev)**2 + (y-y_prev)**2 + (z-z_prev)**2)
                else:
                    k1 = vector_field(x,            y,            perturb=True)
                    k2 = vector_field(x+dt*k1[0]/2, y+dt*k1[1]/2, perturb=True)
                    k3 = vector_field(x+dt*k2[0]/2, y+dt*k2[1]/2, perturb=True)
                    k4 = vector_field(x+dt*k3[0],   y+dt*k3[1],   perturb=True)
...
def rk4_2D(x0, y0, vx, vy, vx_trunc, vy_trunc, num_ensemble, max_len, dt, max_steps, xlim, ylim):
    return rk4_streamline_from_grid(
        x0=x0, y0=y0, z0=None,
        vx=vx, vy=vy, vz=None,
        vx_trunc=vx_trunc, vy_trunc=vy_trunc, vz_trunc=None,
        num_ensemble=num_ensemble,
        max_len=max_len, dt=dt, max_steps=max_steps,
        xlim=xlim, ylim=ylim, zlim=None,
    )


def rk4_3D(x0, y0, z0, vx, vy, vz, vx_trunc, vy_trunc, vz_trunc, num_ensemble, max_len, dt, max_steps, xlim, ylim, zlim):
    return rk4_streamline_from_grid(
        x0=x0, y0=y0, z0=z0,
        vx=vx, vy=vy, vz=vz,
        vx_trunc=vx_trunc, vy_trunc=vy_trunc, vz_trunc=vz_trunc,
        num_ensemble=num_ensemble,
        max_len=max_len, dt=dt, max_steps=max_steps,
...
def parse_seed_points(seed_str, num_dims=2):
    if num_dims == 2:
        ...
        if not matches:
            raise ValueError("Invalid 3D seed format. Use: '(0.1,0.5,0.3),(0.4,0.4,0.6)'")
        x_vals, y_vals, z_vals = zip(*[(float(x), float(y), float(z)) for x, y, z in matches])
...
def parse_arguments():
    ...
    parser.add_argument("--vars", "-v", type=str, required=True,
    ...
    # ---- seeds ----
    parser.add_argument("--seeds_points", "-s", type=str, default=None,
        help="Seed points: 2D '(x1,y1),(x2,y2)' or 3D '(x1,y1,z1),(x2,y2,z2)'")
    parser.add_argument("--num_random_seeds", "-nrs", type=int, default=0,
    ...
    parser.add_argument("--WrightIO", "-wio", type=str, default="writer1",
        help="IO name for the output writer (default: writer1)")

    # ---- output ----
    parser.add_argument("--output", "-o", type=str, default="segments.bp",
        help="Output file name (default: segments.bp)")

    # ---- RK parameters ----
    parser.add_argument("--step_size",    "-dh",   type=np.float64, default=0.001,
        help="RK step size (default: 0.001)")
    parser.add_argument("--num_RK_steps", "-step", type=int,        default=4500,
        help="Number of RK steps per streamline (default: 4500)")
    parser.add_argument("--max_length",   "-MLen", type=float,      default=4.5,
...
def main():
    ...
    # ------------------------------------------------------------------
    # Decide whether ensemble / perturbation mode is active
    # ------------------------------------------------------------------
    need_z_trunc = is_3d  # for 3-D we require a z error file too
    use_ensemble = (
        args.num_ensemble > 1
    ...
    if args.num_ensemble > 1 and not use_ensemble:
        missing = []
        if args.trunc_file_x is None: missing.append("--trunc_file_x")
        if args.trunc_file_y is None: missing.append("--trunc_file_y")
        if need_z_trunc and args.trunc_file_z is None: missing.append("--trunc_file_z")
        print(
    ...
    # ------------------------------------------------------------------
    # Pre-load all steps from the truncation error BP files
    # (done once up front so the main loop just indexes a list)
    # ------------------------------------------------------------------
    ...
    wrigher = WrighterClass.Writer(IO_Name=args.WrightIO, bp_file=args.output, xml=args.xml)
    ...
    while True:
        ...
        # Read velocity fields for this step
        data = []
        ...
        # Pull matching truncation error arrays for this step
        ...
        if not is_3d:
            ...
            coords_x = np.ascontiguousarray(np.array(coords_x, dtype=np.float64)).flatten()
            coords_y = np.ascontiguousarray(np.array(coords_y, dtype=np.float64)).flatten()
            offsets  = np.ascontiguousarray(np.array(offsets,  dtype=np.int32)).flatten()
            ...
            offsets, coords_x, coords_y, coords_z = rk4_3D(
                x_seeds, y_seeds, z_seeds,
                data[0], data[1], data[2],
                vx_trunc, vy_trunc, vz_trunc,
                ne,
                max_len=args.max_length,
                dt=args.step_size,
                max_steps=args.num_RK_steps,
