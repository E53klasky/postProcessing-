...
def curl_3d(vx, vy, vz):
    curl_x = np.gradient(vz, axis=1, edge_order=2) - np.gradient(vy, axis=0, edge_order=2)  
    curl_y = np.gradient(vx, axis=0, edge_order=2) - np.gradient(vz, axis=2, edge_order=2)  
    curl_z = np.gradient(vy, axis=2, edge_order=2) - np.gradient(vx, axis=1, edge_order=2)  
...
def parse_arguments():
    ...
    parser.add_argument("--IO_Name1", type=str, default="reader1", help="IO Name for the first Adios file (default: reader1)")
    parser.add_argument("--writeIO", "-wio", type=str, required=True, help="IO Name for the output Adios file")
    parser.add_argument("--xml", "-x", type=str, default=None, help="Path to ADIOS2 XML configuration file (optional)")
    parser.add_argument("--output", "-o", type=str, default="div_curl.bp", help="Output file name (default: div_curl.bp)")
    parser.add_argument("--vars", "-v", type=str, required=True, help="Velocity variable names (comma-separated, e.g., vx,vy,vz)")
...
def main():
    ...
    reader = ReaderClass.Reader(args.IO_Name1, args.file1, xml=args.xml)
    writer = WrighterClass.Writer(args.writeIO, bp_file=args.output, xml=args.xml)
