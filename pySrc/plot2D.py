...
def parser_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Parallel 2D plot with consistent color scale"
    )
    parser.add_argument(
        "input_file", type=str, help="Path to the BP file to process (REQUIRED)"
    )
    parser.add_argument(
        ...
        for step_count, _ in enumerate(stream):
            ...
            for var in var_list:
                ...
                if not shape or len(shape) != 3 or shape[0] != 1:
                    if rank == 0:
                        print(
                            f"Skipping variable {var} due to unexpected shape {shape}"
                        )
                    continue
                ...
                img_path = save_rank_image(
                    local_data, rank, var, step_count, global_min, global_max
                )
                all_img_paths = comm.gather(img_path, root=0)

                if rank == 0:
                    final_path = os.path.join(
                        output_dir, f"{var}_step_{step_count:04d}.png"
                    )
